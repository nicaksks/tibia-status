...
impl ProtocolGame {
    ...
    pub fn server_status(&self) -> Result<server::Server, (i16, &str)> {
        let addr = &format!("{}:{}", self.ip.clone().unwrap(), self.port.clone().unwrap());
        let conn = TcpStream::connect(addr);

        match conn {
            Ok(server) => {
                let result = self.resultt(server);

                if result.is_empty() {
                    return Err((404, "server.not.found"))
                }
                
                Ok(self.xml_to_json(&self.vec_to_string(result)))?
            }
    ...
    #[allow(unused_must_use)]
    fn resultt(&self, mut server: TcpStream) -> Vec<u8> {
    ...
    fn xml_to_json(&self, xml: &str) -> Result<server::Server, (i16, &str)> {
